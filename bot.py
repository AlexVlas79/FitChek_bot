...
async def adm_block(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    dates = get_available_dates()
    rows = [[InlineKeyboardButton(label, callback_data=f"bld_{ds}")]
...
async def adm_block_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    free = get_free_slots(date_str)
    if not free:
        await query.answer("Нет свободных слотов", show_alert=True); return
    rows = [[InlineKeyboardButton(t, callback_data=f"blt_{t}")] for t in free]
...
async def adm_price_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pc = context.user_data.get("pc", {})
    venue = pc["v"][vi]
    name, _ = pc["i"][venue][ii]
