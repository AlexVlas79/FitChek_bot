...
async def adm_price_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    # data format: adm_pe_{vi}_{ii}
    parts = query.data[7:].split("_")  # vi, ii
    vi, ii = int(parts[0]), int(parts[1])
    pricing = context.user_data.get("pricing_data", {})
    venues = list(pricing.keys())
    venue = venues[vi]
...
async def adm_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not action:
        return  # Not in admin input mode, ignore
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(adm_extra_date,     pattern=r"^adm_exd_"),      group=0)
    app.add_handler(CallbackQueryHandler(adm_price_edit,     pattern=r"^adm_pe_"),       group=0)
    app.add_handler(CallbackQueryHandler(adm_fee_edit,       pattern=r"^adm_fe_"),       group=0)
    app.add_handler(CallbackQueryHandler(adm_add_admin_start,pattern="^adm_addadmin$"),  group=0)

    # ConversationHandler для текстового ввода в панели тренера
    app.add_handler(booking_conv)
    app.add_handler(contact_conv)
    # Admin text input — only fires when adm_action is set, after conv handlers
